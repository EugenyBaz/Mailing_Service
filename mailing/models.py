...
class Mailing(models.Model):
    ...
    date_and_time_first_launched = models.DateTimeField(
        verbose_name="Дата и время первой отправки", blank=True, null=True
    )
    date_and_time_finish_launched = models.DateTimeField(
    ...
    def check_expiration(self):
        if (
            self.date_and_time_finish_launched
            and self.date_and_time_finish_launched <= now()
    ...
    def _send_email(self):
        ...
        attempt = AttemptMailing.objects.create(status=status, server_response=server_response, mailing=self)
